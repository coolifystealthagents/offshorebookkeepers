...
const root = (process.env.SEP8_BASE_URL || 'http://127.0.0.1:3000').replace(/\/$/, '');
const directory = '.paperclip/daily-content/2026-09-08';
...
for (const record of records) {
  ...
  if (!source.includes('published: "2026-09-08"') || !source.includes('September 8, 2026')) fail(`${record.sourcePath} has incorrect dates`);
...
for (const record of records) {
  ...
  if (!html.includes('September 8, 2026')) fail(`${record.route} lacks visible date`);
  if (!html.includes('"datePublished":"2026-09-08"')) fail(`${record.route} lacks structured datePublished`);
