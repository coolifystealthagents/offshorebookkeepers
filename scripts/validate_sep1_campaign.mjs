...
const root = process.env.SEP1_BASE_URL || 'http://127.0.0.1:3000';
const dir = '.paperclip/daily-content/2026-09-01';
const blog = JSON.parse(fs.readFileSync(`${dir}/blog.json`, 'utf8'));
const research = JSON.parse(fs.readFileSync(`${dir}/research.json`, 'utf8'));
...
const hashes = records.map((record) => crypto.createHash('sha256').update(fs.readFileSync(record.sourcePath, 'utf8')).digest('hex'));
...
for (const record of records) {
  ...
  if (!source.includes('published: "2026-09-01"') || !source.includes('September 1, 2026')) fail(`${record.sourcePath} has incorrect date fields`);
  const image = source.match(/^featuredImage: "([^"]+)"/m)?.[1];
  if (!image || !fs.existsSync(`public${image}`)) fail(`${record.sourcePath} does not use an existing asset`);
}
...
for (const record of records) {
  ...
  if (!html.includes('September 1, 2026')) fail(`${record.route} lacks visible date`);
  if (!html.includes('"datePublished":"2026-09-01"')) fail(`${record.route} lacks structured datePublished`);
  ...
  if (!index.includes(`href="${record.route}"`)) fail(`${record.route} is absent from its index`);
  if (!sitemap.includes(`<loc>${record.canonical}</loc>`)) fail(`${record.route} is absent from sitemap`);
}
console.log(`PASS: ${blog.length} Blog + ${research.length} Research; unique metadata and hashes; 17 HTTP 200 self-canonical routes with visible/structured dates, index links, sitemap entries, and existing assets.`);
