...
test('homepage chrome emits detail links only from the approved service catalog', () => { const s = fs.readFileSync(path.join(root,'app/components.tsx'),'utf8'); for (const slug of ['admin-support','customer-support','operations-support','reporting-and-qa']) assert.ok(!s.includes(`/services/${slug}`), slug); assert.match(s,/import \{ fleetServices \} from '\.\/fleet-data'/); assert.match(s,/fleetServices\.slice\(0,6\)\.map\(s=><a href=\{`\/services\/\$\{s\.slug\}`\}/); });
