---
title: "How to study prepaid-schedule exceptions"
description: "A reproducible study protocol for whether prepaid-expense schedules remain traceable to source documents, ledger balances, service periods, and approved recognition rules, with frozen populations, evidence coverage, decision rights, and limitations."
...
The practical question is whether prepaid-expense schedules remain traceable to source documents, ledger balances, service periods, and approved recognition rules. For this protocol, the observation unit is one prepaid-schedule line for one reporting period. The primary event is a line that fails a defined tie-out, source-support, service-period, or approval test at cutoff. Those definitions must be approved before records are examined. Otherwise, a team can improve a result merely by excluding difficult items, moving a cutoff, or changing when the clock starts.
...
Select one workflow, one entity or an explicitly listed entity group, and consecutive periods. Avoid a handpicked clean week. Each study row should contain schedule line ID, entity, account, vendor token, invoice ID, original amount, service dates, current amortization, remaining balance, ledger balance, reviewer, and evidence link. Stable identifiers matter: updates to an existing item must not create a second apparent observation, while two genuinely separate events must not be collapsed because their amounts happen to match.
...
Create a codebook with observable tests for eligible, achieved, open, returned, excluded, and indeterminate. For this topic, the exception map should explicitly cover renewals, cancellations, credits, partial service periods, foreign currency, reclassifications, duplicate lines, missing contracts, and post-close adjustments. Each state needs required evidence, a decision owner, and a rule for conflicting records. --Done-- is not usable evidence unless the supporting record, decision, and timestamp can be traced.
