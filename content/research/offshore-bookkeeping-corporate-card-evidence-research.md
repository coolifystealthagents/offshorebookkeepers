...
description: "A reproducible study protocol for whether corporate-card transactions have timely receipts, business-purpose records, coding support, approval evidence, and a traceable ledger posting, with frozen populations, evidence coverage, decision rights, and limitations."
...
The practical question is whether corporate-card transactions have timely receipts, business-purpose records, coding support, approval evidence, and a traceable ledger posting. For this protocol, the observation unit is one posted corporate-card transaction for one card program, entity, and statement period. The primary event is a transaction that lacks one or more evidence elements required by the approved policy at cutoff. Those definitions must be approved before records are examined. Otherwise, a team can improve a result merely by excluding difficult items, moving a cutoff, or changing when the clock starts.

For a business considering Philippines-based bookkeeping support, this is a workflow-design question rather than a claim about geography. A remote bookkeeper can assemble authorized exports, maintain the study table, apply deterministic rules, and route exceptions. The controller, expense-policy owner, or authorized approver keeps decisions that affect accounting treatment, policy, approval, or release. A sound staffing scope names the inputs, preparation steps, stop conditions, reviewer, closure evidence, and escalation deadline.
...
The denominator is every eligible one posted corporate-card transaction for one card program, entity, and statement period in consecutive periods. The primary numerator is every record meeting this event: a transaction that lacks one or more evidence elements required by the approved policy at cutoff. Retain open, excluded, and indeterminate records in separate tables with reasons. Never remove a record because its support is inconvenient or because it arrived late. The authorized finance owner should approve eligibility and exception rules before the pilot.
...
Select one workflow, one entity or an explicitly listed entity group, and consecutive periods. Avoid a handpicked clean week. Each study row should contain transaction ID, entity, card token, merchant, transaction date, posting date, amount, currency, receipt status, business purpose, coding, approval status, ledger posting, reviewer, and evidence link. Stable identifiers matter: updates to an existing item must not create a second apparent observation, while two genuinely separate events must not be collapsed because their amounts happen to match.
...
Create a codebook with observable tests for eligible, achieved, open, returned, excluded, and indeterminate. For this topic, the exception map should explicitly cover credits, disputed charges, recurring subscriptions, split transactions, foreign exchange, personal reimbursements, card replacements, late receipts, and statement timing differences. Each state needs required evidence, a decision owner, and a rule for conflicting records. --Done-- is not usable evidence unless the supporting record, decision, and timestamp can be traced.
