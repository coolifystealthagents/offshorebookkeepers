---
title: "A defensible reconciling-item clearance rate"
description: "A field-tested study design for following open reconciliation items from first identification to supported resolution."
...
The practical question is how a business can study a reconciling item first recorded in a defined reconciliation population without confusing activity with control quality. In this protocol, the measured event occurs when the item reaches a supported closed state under the prewritten closure rule. The study is useful only when the organization fixes that event definition before looking at results. A faster clearance rate is not automatically better; unsupported write-offs or premature closure can make it look better.

This distinction matters for a business considering offshore bookkeeping support. A remote bookkeeper can assemble authorized records, maintain the study table, apply a written classification, and raise missing evidence. The account owner or reconciliation reviewer retains the decision about whether the difference is timing, error, missing evidence, or another approved class. Location does not change that boundary. A well-designed handoff makes the source, preparation, open question, decision owner, and final evidence visible to both sides.
...
Ask one narrow question: within a fixed population and period, what share of a reconciling item first recorded in a defined reconciliation population meets the defined event, how long does the transition take where time is relevant, and which documented exception classes explain the remainder? Write the numerator, denominator, observation unit, clock, cutoff, time zone, status values, and treatment of reopened items before collecting outcomes.

The base denominator is every eligible a reconciling item first recorded in a defined reconciliation population that enters the frozen population. The numerator is the subset for which the item reaches a supported closed state under the prewritten closure rule. Retain excluded records in a separate table with a reason; do not delete them from the audit trail. Proposed exclusions are items carried only as labels without an amount or source and entries outside the frozen account population. A finance owner should approve these rules before the pilot and approve any change prospectively.
...
Select one workflow, one entity or clearly listed group of entities, and consecutive periods. Avoid cherry-picking a clean month. The minimum study row should contain item ID, account, amount, first-seen date, expected clearing date, source link, owner, status history, review date, and closure evidence. Use stable study identifiers so updates do not create apparent new items. Preserve the raw export, extraction time, report parameters, and file hash when practical.
...
The bookkeeper may apply deterministic rules and flag uncertainty. The account owner or reconciliation reviewer resolves questions involving whether the difference is timing, error, missing evidence, or another approved class. Do not ask the preparer to infer authorization from tone, repeat an old treatment without current evidence, or change a classification to meet a target. Those shortcuts make the dataset look complete while weakening its meaning.
...
Break the results down by account, age band, cause class, value band, source system, and assigned owner, but only when groups are large enough to avoid exposing an individual or customer. Label small cells as suppressed under a prewritten rule. A difference between groups is descriptive. It does not establish that staffing model, country, software, or individual performance caused the difference. Volume, complexity, deadline timing, missing source records, migrations, outages, policy changes, and reviewer availability are plausible confounders.
