---
title: "How to study intercompany confirmation mismatches"
description: "A reproducible study protocol for whether reciprocal intercompany balances are confirmed, explained, and resolved before consolidation or reporting release, with frozen populations, evidence coverage, decision rights, and limitations."
...
Select one workflow, one entity or an explicitly listed entity group, and consecutive periods. Avoid a handpicked clean week. Each study row should contain period, entity pair, account pair, currency, sender balance, receiver balance, difference, transaction references, confirmation date, resolution status, reviewer, and evidence link. Stable identifiers matter: updates to an existing item must not create a second apparent observation, while two genuinely separate events must not be collapsed because their amounts happen to match.
...
Create a codebook with observable tests for eligible, achieved, open, returned, excluded, and indeterminate. For this topic, the exception map should explicitly cover timing differences, foreign-exchange rates, unmatched recharges, cash in transit, differing charts of accounts, late journals, eliminations, and disputed allocations. Each state needs required evidence, a decision owner, and a rule for conflicting records. --Done-- is not usable evidence unless the supporting record, decision, and timestamp can be traced.
