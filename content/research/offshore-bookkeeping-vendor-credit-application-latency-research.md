...
The practical question is whether approved vendor credits are applied to eligible invoices promptly or remain open without a supported disposition. For this protocol, the observation unit is one vendor credit memo or credit balance. The primary event is elapsed time from evidence-complete credit recognition to supported application, refund, or authorized carryforward. Those definitions must be approved before records are examined. Otherwise, a team can improve a result merely by excluding difficult items, moving a cutoff, or changing when the clock starts.
...
The denominator is every eligible one vendor credit memo or credit balance in consecutive periods. The primary numerator is every record meeting this event: elapsed time from evidence-complete credit recognition to supported application, refund, or authorized carryforward. Retain open, excluded, and indeterminate records in separate tables with reasons. Never remove a record because its support is inconvenient or because it arrived late. The authorized finance owner should approve eligibility and exception rules before the pilot.
...
Select one workflow, one entity or an explicitly listed entity group, and consecutive periods. Avoid a handpicked clean week. Each study row should contain credit ID, entity, vendor token, credit date, amount, currency, source invoice, eligible invoice, application date, disposition, reviewer, and evidence link. Stable identifiers matter: updates to an existing item must not create a second apparent observation, while two genuinely separate events must not be collapsed because their amounts happen to match.
...
Create a codebook with observable tests for eligible, achieved, open, returned, excluded, and indeterminate. For this topic, the exception map should explicitly cover disputed credits, cross-entity balances, future purchases, refunds in transit, duplicate credits, partial applications, currency differences, and unavailable invoices. Each state needs required evidence, a decision owner, and a rule for conflicting records. --Done-- is not usable evidence unless the supporting record, decision, and timestamp can be traced.
