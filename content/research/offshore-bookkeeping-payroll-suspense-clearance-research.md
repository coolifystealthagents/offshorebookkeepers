---
title: "How to study payroll-suspense clearance"
description: "A reproducible study protocol for whether payroll-related suspense and clearing balances are resolved with evidence before the close or simply carried forward, with frozen populations, evidence coverage, decision rights, and limitations."
...
Select one workflow, one entity or an explicitly listed entity group, and consecutive periods. Avoid a handpicked clean week. Each study row should contain payroll run, entity, pay date, source-system ID, ledger line, amount, account, employee token where permitted, jurisdiction, proposed mapping, resolution date, reviewer, and evidence link. Stable identifiers matter: updates to an existing item must not create a second apparent observation, while two genuinely separate events must not be collapsed because their amounts happen to match.
...
Create a codebook with observable tests for eligible, achieved, open, returned, excluded, and indeterminate. For this topic, the exception map should explicitly cover provider-to-ledger differences, returned wages, benefit deductions, tax withdrawals, garnishments, rounding, off-cycle payroll, reversals, and restricted employee data. Each state needs required evidence, a decision owner, and a rule for conflicting records. --úDone--ù is not usable evidence unless the supporting record, decision, and timestamp can be traced.
