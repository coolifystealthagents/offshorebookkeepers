---
title: "How to measure the age of evidence gaps in reconciliations"
description: "A reproducible research framework for reconciliation evidence-gap aging, with defined measures, inference limits, safeguards, and references."
...
This brief asks how long a defined missing-evidence condition remains open between first detection and documented resolution. The outcome depends on a visible event rule, not on a reviewer's impression after seeing the records.

The public sources below support traceable records, controlled access, documentation, and reviewable evidence. Applying those principles to reconciliation evidence-gap aging is OffshoreBookkeepers.com analysis. The cited organizations do not report or endorse the proposed measure.

## Methodology

Select one defined workflow and a fixed series of consecutive periods. Treat one request, invoice, reconciliation, vendor change, or close task as the observation unit. Capture reconciliation ID, evidence request, detection time, owner, due time, response time, sufficiency result, and closure time. Record the entity, period, source system, time zone, and authorized reviewer. Write inclusion, exclusion, start, stop, and reopen rules before inspecting results.
