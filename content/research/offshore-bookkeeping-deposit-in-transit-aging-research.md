---
title: "How to study deposit-in-transit aging in bank reconciliation"
description: "A reproducible study protocol for whether recorded receipts reach the bank statement within an expected window or remain unresolved across reconciliation cutoffs, with frozen populations, evidence coverage, decision rights, and limitations."
...
Select one workflow, one entity or an explicitly listed entity group, and consecutive periods. Avoid a handpicked clean week. Each study row should contain deposit ID, bank account, entity, ledger date, amount, currency, receipt channel, bank posting date, matching status, exception reason, owner, and evidence link. Stable identifiers matter: updates to an existing item must not create a second apparent observation, while two genuinely separate events must not be collapsed because their amounts happen to match.
...
Create a codebook with observable tests for eligible, achieved, open, returned, excluded, and indeterminate. For this topic, the exception map should explicitly cover weekend batches, processor settlement lags, split deposits, net settlements, reversals, duplicate records, bank-feed gaps, and post-cutoff matches. Each state needs required evidence, a decision owner, and a rule for conflicting records. --Done-- is not usable evidence unless the supporting record, decision, and timestamp can be traced.
