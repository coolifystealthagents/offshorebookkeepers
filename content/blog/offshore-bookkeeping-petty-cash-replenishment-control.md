---
title: "Control petty cash replenishment without losing receipt-level evidence"
description: "A practical workflow for petty cash replenishments, evidence control, exceptions, and owner review."
...
A useful petty cash replenishments workpaper must let a reviewer reconstruct what entered the schedule, what changed, and what remains open. Record the entity, account, period, currency, source systems, preparer, reviewer, and cutoff. For this workflow, make the custody count the named control focus. The bookkeeper owns disciplined assembly; the cash custodian keeps authority over allowability and replenishment approval.
...
The expected packet includes custodian count, receipt log, approval, replenishment request, bank activity, and ledger posting. Give each record a stable reference and capture source, effective date, amount, currency, owner, and approved storage location. Mark unavailable evidence as missing rather than substituting an assumption. Sensitive files should stay in access-controlled systems.
...
The preparer may reproduce arithmetic, compare observable fields, and draft a question. The preparer should not invent support or decide allowability and replenishment approval. Route that question to the cash custodian with the evidence reference and the exact decision requested. Record the response and approval under the client policy.
...
Retain the scope statement, original exports, evidence index, movement bridge, exception register, reviewer notes, approvals, and change log. Carry forward only genuinely open items. A complete petty cash replenishments packet explains the population, evidence, movements, exceptions, and decisions without requiring the next reviewer to reconstruct the story from messages.
