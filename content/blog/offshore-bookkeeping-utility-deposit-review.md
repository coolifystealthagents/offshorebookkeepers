---
title: "Review utility deposits and refunds with account-level evidence"
description: "A practical workflow for utility deposits, evidence control, exceptions, and owner review."
...
A useful utility deposits workpaper must let a reviewer reconstruct what entered the schedule, what changed, and what remains open. Record the entity, account, period, currency, source systems, preparer, reviewer, and cutoff. For this workflow, make the recovery log the named control focus. The bookkeeper owns disciplined assembly; the facilities owner keeps authority over recoverability and classification.
...
The expected packet includes provider statement, service agreement, payment confirmation, refund correspondence, bank activity, and ledger detail. Give each record a stable reference and capture source, effective date, amount, currency, owner, and approved storage location. Mark unavailable evidence as missing rather than substituting an assumption. Sensitive files should stay in access-controlled systems.
...
The preparer may reproduce arithmetic, compare observable fields, and draft a question. The preparer should not invent support or decide recoverability and classification. Route that question to the facilities owner with the evidence reference and the exact decision requested. Record the response and approval under the client policy.
...
Retain the scope statement, original exports, evidence index, movement bridge, exception register, reviewer notes, approvals, and change log. Carry forward only genuinely open items. A complete utility deposits packet explains the population, evidence, movements, exceptions, and decisions without requiring the next reviewer to reconstruct the story from messages.
