---
title: "Reconcile franchise advertising fund collections and spending"
description: "A practical workflow for franchise advertising funds, source control, exception handling, and owner review."
...
Advertising funds need a clean boundary between money collected and costs approved for the fund. A usable franchise advertising funds file shows what entered the schedule, what moved, and what still needs an answer. Record the entity, period, currency, source systems, preparer, reviewer, and cutoff at the top.
...
For this workflow, collect franchisee billing, cash receipt, campaign invoice, allocation schedule, approval record, and fund ledger. Give each item a stable reference. Capture its source, date, amount, currency, status, and owner. If a record is missing, label it missing and request it. An estimate or a screenshot from an unapproved message thread is not a substitute.
...
The preparer can check arithmetic, compare observable fields, and draft a focused question. The preparer does not decide allowable use and allocation policy. Route that question to the franchise finance owner, along with the evidence reference and the date a response is needed. Record the answer in the workpaper, not only in chat.
...
Deliver the scope note, original exports, evidence index, movement bridge, exception list, approvals, and change log together. Carry forward only items that remain open. The reviewer should be able to understand the franchise advertising funds position without reconstructing the file from email or memory.
