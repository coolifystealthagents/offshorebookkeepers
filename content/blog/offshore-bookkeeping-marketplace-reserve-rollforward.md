---
title: "Build a marketplace reserve rollforward that reviewers can retrace"
description: "A practical workflow for marketplace reserves, source control, exception handling, and owner review."
...
A reserve schedule can tie to the ledger and still leave a reviewer guessing which payout caused a movement. A usable marketplace reserves file shows what entered the schedule, what moved, and what still needs an answer. Record the entity, period, currency, source systems, preparer, reviewer, and cutoff at the top.
...
For this workflow, collect seller statement, reserve activity export, payout report, dispute log, bank receipt, and general ledger detail. Give each item a stable reference. Capture its source, date, amount, currency, status, and owner. If a record is missing, label it missing and request it. An estimate or a screenshot from an unapproved message thread is not a substitute.
...
The preparer can check arithmetic, compare observable fields, and draft a focused question. The preparer does not decide reserve classification and release treatment. Route that question to the commerce finance owner, along with the evidence reference and the date a response is needed. Record the answer in the workpaper, not only in chat.
...
Deliver the scope note, original exports, evidence index, movement bridge, exception list, approvals, and change log together. Carry forward only items that remain open. The reviewer should be able to understand the marketplace reserves position without reconstructing the file from email or memory.
