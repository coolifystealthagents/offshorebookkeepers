...
sources: [{"name":"AIA G702/G703 overview","url":"https://www.aiacontracts.com/articles/183616-g702-1992-application-and-certificate-for-payment/"},{"name":"FAR progress payment rules","url":"https://www.acquisition.gov/far/32.503-1"}]
...
A construction pay application may include materials purchased but not yet installed. The billed amount can depend on contract terms, ownership evidence, storage location, insurance, prior billing, and retainage. A simple copy of the supplier invoice does not prove every condition.
...
Useful background for designing the procedure includes [AIA G702/G703 overview](https://www.aiacontracts.com/articles/183616-g702-1992-application-and-certificate-for-payment/) and [FAR progress payment rules](https://www.acquisition.gov/far/32.503-1). These sources do not replace the company’s contracts, accounting policy, or professional advice. Record the version and access date when a changing rule or provider document affects the workflow.
...
The working table should contain project, pay application, schedule-of-values line, supplier invoice, material description, storage site, received date, insurance evidence, prior billed amount, current requested amount, retainage, and reviewer status. Use stable references instead of names where confidentiality requires it. Never delete unmatched rows merely because they make the schedule untidy. A missing record is an exception with an owner, not permission to invent a value.
...
## Build the stored-materials billing check
...
The offshore bookkeeper can download approved reports, compare identifiers and amounts, reperform arithmetic, request missing documents, update statuses, and draft a proposed entry for review. The bookkeeper should stop when the work requires whether contract conditions for billing stored materials are met and whether the application should be certified. That decision belongs to the project manager and authorized contract reviewer.
