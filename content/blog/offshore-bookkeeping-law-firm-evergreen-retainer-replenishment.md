---
title: "Reconcile law-firm evergreen retainer replenishments by matter"
description: "Connect trust balances, earned-fee transfers, client invoices, replenishment requests, and matter-level approvals."
...
An evergreen retainer requires a firm to compare the agreed minimum with the matter’s current trust balance after authorized fee transfers and client activity. A generic request for funds can overstate the replenishment or lose the link to the client ledger.
...
Useful background for designing the procedure includes [ABA Model Rule 1.15](https://www.americanbar.org/groups/professional_responsibility/publications/model_rules_of_professional_conduct/rule_1_15_safekeeping_property/) and [ABA client protection resources](https://www.americanbar.org/groups/professional_responsibility/resources/client-protection/). These sources do not replace the company’s contracts, accounting policy, or professional advice. Record the version and access date when a changing rule or provider document affects the workflow.
...
The working table should contain matter, client reference, agreed minimum, opening trust balance, deposit, authorized transfer, refund, current balance, replenishment calculation, invoice, attorney approval, and response status. Use stable references instead of names where confidentiality requires it. Never delete unmatched rows merely because they make the schedule untidy. A missing record is an exception with an owner, not permission to invent a value.
...
## Build the evergreen retainer reconciliation
...
A matter requires a $10,000 minimum. The opening trust balance is $11,500, an approved $4,200 transfer posts, and a $1,000 replenishment arrives before cutoff. The supported request is $1,700, subject to the engagement terms and attorney review.
...
The offshore bookkeeper can download approved reports, compare identifiers and amounts, reperform arithmetic, request missing documents, update statuses, and draft a proposed entry for review. The bookkeeper should stop when the work requires fee entitlement, transfer authorization, request timing, and jurisdiction-specific trust rules. That decision belongs to the responsible attorney or trust-account supervisor.
