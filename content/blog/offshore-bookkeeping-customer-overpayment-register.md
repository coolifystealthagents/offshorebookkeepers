---
title: "Build a customer overpayment register with clear resolution paths"
description: "A practical workflow for customer overpayments, evidence control, exceptions, and owner review."
...
A useful customer overpayments workpaper must let a reviewer reconstruct what entered the schedule, what changed, and what remains open. Record the entity, account, period, currency, source systems, preparer, reviewer, and cutoff. For this workflow, make the aging bridge the named control focus. The bookkeeper owns disciplined assembly; the accounts receivable owner keeps authority over refund, application, or escheatment decision.
...
The expected packet includes remittance detail, customer ledger, deposit record, correspondence, and refund approval. Give each record a stable reference and capture source, effective date, amount, currency, owner, and approved storage location. Mark unavailable evidence as missing rather than substituting an assumption. Sensitive files should stay in access-controlled systems.
...
The preparer may reproduce arithmetic, compare observable fields, and draft a question. The preparer should not invent support or decide refund, application, or escheatment decision. Route that question to the accounts receivable owner with the evidence reference and the exact decision requested. Record the response and approval under the client policy.
...
Retain the scope statement, original exports, evidence index, movement bridge, exception register, reviewer notes, approvals, and change log. Carry forward only genuinely open items. A complete customer overpayments packet explains the population, evidence, movements, exceptions, and decisions without requiring the next reviewer to reconstruct the story from messages.
