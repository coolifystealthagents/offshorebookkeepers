---
title: "Reconcile referral fees from source event to settlement"
description: "A practical workflow for referral fees, evidence control, exceptions, and owner review."
...
A useful referral fees workpaper must let a reviewer reconstruct what entered the schedule, what changed, and what remains open. Record the entity, account, period, currency, source systems, preparer, reviewer, and cutoff. For this workflow, make the contract register the named control focus. The bookkeeper owns disciplined assembly; the commercial owner keeps authority over eligibility and contract interpretation.
...
The expected packet includes partner agreement, eligible-event report, approved rate table, invoice, settlement, and ledger entry. Give each record a stable reference and capture source, effective date, amount, currency, owner, and approved storage location. Mark unavailable evidence as missing rather than substituting an assumption. Sensitive files should stay in access-controlled systems.
...
The preparer may reproduce arithmetic, compare observable fields, and draft a question. The preparer should not invent support or decide eligibility and contract interpretation. Route that question to the commercial owner with the evidence reference and the exact decision requested. Record the response and approval under the client policy.
...
Retain the scope statement, original exports, evidence index, movement bridge, exception register, reviewer notes, approvals, and change log. Carry forward only genuinely open items. A complete referral fees packet explains the population, evidence, movements, exceptions, and decisions without requiring the next reviewer to reconstruct the story from messages.
