---
title: "Assemble an import duty landed-cost packet for inventory review"
description: "A practical workflow for import duty and landed cost, source control, exception handling, and owner review."
...
Landed cost work depends on matching shipment evidence before anyone chooses an allocation method. A usable import duty and landed cost file shows what entered the schedule, what moved, and what still needs an answer. Record the entity, period, currency, source systems, preparer, reviewer, and cutoff at the top.
...
The preparer can check arithmetic, compare observable fields, and draft a focused question. The preparer does not decide cost allocation method and capitalization. Route that question to the inventory accounting owner, along with the evidence reference and the date a response is needed. Record the answer in the workpaper, not only in chat.
...
Deliver the scope note, original exports, evidence index, movement bridge, exception list, approvals, and change log together. Carry forward only items that remain open. The reviewer should be able to understand the import duty and landed cost position without reconstructing the file from email or memory.
