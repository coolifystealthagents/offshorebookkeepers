---
title: "Maintain a construction mobilization advance ledger by job"
description: "A practical workflow for construction mobilization advances, source control, exception handling, and owner review."
...
A job advance is easy to post and surprisingly easy to lose inside progress billing. A usable construction mobilization advances file shows what entered the schedule, what moved, and what still needs an answer. Record the entity, period, currency, source systems, preparer, reviewer, and cutoff at the top.
...
For this workflow, collect signed contract, payment application, bank receipt, project ledger, billing schedule, and approval record. Give each item a stable reference. Capture its source, date, amount, currency, status, and owner. If a record is missing, label it missing and request it. An estimate or a screenshot from an unapproved message thread is not a substitute.
...
The preparer can check arithmetic, compare observable fields, and draft a focused question. The preparer does not decide earned status and revenue treatment. Route that question to the construction finance owner, along with the evidence reference and the date a response is needed. Record the answer in the workpaper, not only in chat.
...
Deliver the scope note, original exports, evidence index, movement bridge, exception list, approvals, and change log together. Carry forward only items that remain open. The reviewer should be able to understand the construction mobilization advances position without reconstructing the file from email or memory.
